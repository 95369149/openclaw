...
async function loadSession(id: string): Promise<SessionRecord | null> {
  ...
  try {
    ...
    const sid =
      (typeof (j as any).id === "string" && (j as any).id.trim()) ||
      (typeof (j as any).sessionId === "string" && (j as any).sessionId.trim()) ||
      id;
    const metadata = normalizeSessionMetadata((j as any).metadata ?? (j as any).chatMetadata ?? j);
    const messages = Array.isArray((j as any).messages)
      ? ((j as any).messages as SessionRecord["messages"])
      : [];
    const createdAt =
    ...
    const updatedAt =
      typeof (j as any).updatedAt === "string" ? ((j as any).updatedAt as string) : createdAt;
...
async function main(): Promise<void> {
  ...
  try {
    ...
    if (args.json) {
      console.log(
        formatJson(out, { savedImage, sessionId: args.sessionId, model: model.model_name }),
      );
