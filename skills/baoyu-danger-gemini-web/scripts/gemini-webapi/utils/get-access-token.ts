...
async function fetch_google_extra_cookies(
  proxy: string | null,
  verbose: boolean,
  ...
  try {
    ...
    if (verbose)
      logger.debug(`Skipping google.com preflight: ${e instanceof Error ? e.message : String(e)}`);
  }
  return {};
}
...
export async function get_access_token(
  ...
  const forceLogin = !!(
    process.env.GEMINI_WEB_LOGIN?.trim() || process.env.GEMINI_WEB_FORCE_LOGIN?.trim()
  );
  const shouldUseChromeFirst =
    forceLogin ||
    (!cachedFile && !base_cookies["__Secure-1PSID"] && !base_cookies["__Secure-1PSIDTS"]);
  ...
  if (shouldUseChromeFirst) {
    try {
      ...
      if (verbose)
        logger.warning(
          `Failed to load cookies via Chrome CDP: ${e instanceof Error ? e.message : String(e)}`,
        );
    }
  }
  ...
  if (base_cookies["__Secure-1PSID"] && !base_cookies["__Secure-1PSIDTS"]) {
    ...
    for (const c of caches) {
      candidates.push(
        merge_cookie_maps(extra, { "__Secure-1PSID": c.sid, "__Secure-1PSIDTS": c.sidts }),
      );
    }
  ...
  const try_candidates = async (): Promise<[string, Record<string, string>]> => {
    ...
    const attempts = unique.map(async (c, i) => {
      try {
        ...
        if (verbose)
          logger.debug(
            `Init attempt (${i + 1}/${unique.length}) failed: ${e instanceof Error ? e.message : String(e)}`,
          );
        throw e;
      }
    });
  ...
  for (const cookies of Object.values(browser)) {
    ...
    if (
      base_cookies["__Secure-1PSID"] &&
      cookies["__Secure-1PSID"] &&
      cookies["__Secure-1PSID"] !== base_cookies["__Secure-1PSID"]
    ) {
      if (verbose)
        logger.debug(
          "Skipping loaded browser cookies: __Secure-1PSID does not match the one provided.",
        );
      continue;
    }
