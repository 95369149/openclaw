...
export class GeminiClient extends GemMixin {
  ...
  async start_auto_refresh(signal: AbortSignal): Promise<void> {
    while (!signal.aborted) {
      ...
      try {
        ...
        logger.warning(
          `Unexpected error while refreshing cookies: ${e instanceof Error ? e.message : String(e)}`,
        );
      }
  ...
  async generate_content(
    ...
    return await this._run(async () => {
      ...
      if (typeof model === "string") mdl = Model.from_name(model);
        ...
        throw new TypeError(
          `'model' must be a Model instance, string, or dictionary; got ${typeof model}`,
        );
      ...
      const uploaded =
        ...
        (await Promise.all(
          f.map(
            async (p) =>
              [[await upload_file(p, this.proxy)], parse_file_name(p)] as [string[], string],
          ),
        ));
      ...
      try {
        const timeout_ms =
          typeof kwargs.timeout_ms === "number" ? kwargs.timeout_ms : this.timeout * 1000;
      ...
      if (res.status !== 200) {
        await this.close();
        throw new APIError(
          `Failed to generate contents. Request failed with status code ${res.status}`,
        );
      }
        ...
        for (let candidate_index = 0; candidate_index < candidate_list.length; candidate_index++) {
          ...
          for (const w of get_nested_value<unknown[]>(candidate, [12, 1], [])) {
            ...
            web_images.push(
              new WebImage(
                url,
                String(get_nested_value(w, [7, 0], "")),
                String(get_nested_value(w, [0, 4], "")),
                this.proxy,
              ),
            );
          }
          ...
          if (wants_generated) {
            ...
            for (
              let part_index = body_index;
              part_index < (response_json as unknown[]).length;
              part_index++
            ) {
            ...
            for (let img_index = 0; img_index < gen.length; img_index++) {
              ...
              const alt =
                (typeof alt_list[img_index] === "string"
                  ? (alt_list[img_index] as string)
                  : null) ??
            ...
            if (generated_images.length === 0) {
              ...
              for (const url of urls) {
                generated_images.push(
                  new GeneratedImage(url, "[Generated Image]", "", this.proxy, this.cookies),
                );
              }
            }
          }
  ...
  async generateContent(
    ...
    return await this.generate_content(
      prompt,
      files ?? null,
...
export class ChatSession {
  ...
  async send_message(
    ...
    return await this.geminiclient.generate_content(
      prompt,
      files,
      this.model,
      this.gem,
