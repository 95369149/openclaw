...
export async function publishArticle(options: ArticleOptions): Promise<void> {
  ...
  if (title.length > TITLE_MAX_LENGTH) {
    console.warn(
      `[weibo-article] Title exceeds ${TITLE_MAX_LENGTH} chars (${title.length}), truncating at word boundary...`,
    );
    ...
    title =
      lastBreak > TITLE_MAX_LENGTH * 0.4
  ...
  if (summary.length > SUMMARY_MAX_LENGTH) {
    console.warn(
      `[weibo-article] Summary exceeds ${SUMMARY_MAX_LENGTH} chars (${summary.length}), regenerating from content...`,
    );
  ...
  try {
    ...
    if (writeButtonFound) {
      ...
      if (!editable) {
        console.warn(
          "[weibo-article] Title input still readonly after waiting. Proceeding anyway...",
        );
      }
    } else {
      // Maybe we're already on the editor page
      console.log(
        '[weibo-article] "写文章" button not found, checking if editor is already loaded...',
      );
    ...
    if (title) {
      ...
      const titleExists = await cdp.send<{ result: { value: boolean } }>(
        "Runtime.evaluate",
        {
      ...
      if (!titleExists.result.value) {
        ...
        if (titleCheck.result.value === title) {
          ...
          console.warn(
            `[weibo-article] Title partially entered: "${titleCheck.result.value}" (expected: "${title}")`,
          );
        } else {
          console.warn(
            "[weibo-article] Title input appears empty after insertion, trying execCommand fallback...",
          );
          await cdp.send(
            "Runtime.evaluate",
            {
          ...
          const titleRecheck = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `document.querySelector('textarea[placeholder="请输入标题"]')?.value || ''`,
              returnByValue: true,
    ...
    if (summary) {
      ...
      const summaryExists = await cdp.send<{ result: { value: boolean } }>(
        "Runtime.evaluate",
        {
      ...
      if (!summaryExists.result.value) {
        console.error(
          '[weibo-article] Summary input NOT found: textarea[placeholder="导语（选填）"]',
        );
      } else {
        ...
        const summaryCheck = await cdp.send<{ result: { value: string } }>(
          "Runtime.evaluate",
          {
        ...
        if (summaryCheck.result.value === summary) {
          ...
          console.warn(
            "[weibo-article] Summary input appears empty, trying execCommand fallback...",
          );
          await cdp.send(
            "Runtime.evaluate",
            {
          ...
          const summaryRecheck = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `document.querySelector('textarea[placeholder="导语（选填）"]')?.value || ''`,
              returnByValue: true,
    ...
    const editorExists2 = await cdp.send<{ result: { value: string } }>(
      "Runtime.evaluate",
      {
        expression: `(() => {
    ...
    const contentCheck = await cdp.send<{ result: { value: number } }>(
      "Runtime.evaluate",
      {
    ...
    if (contentCheck.result.value > 50) {
      console.log(
        `[weibo-article] Content inserted via clipboard paste (${contentCheck.result.value} chars)`,
      );
    } else {
      console.log(
        `[weibo-article] Clipboard paste got ${contentCheck.result.value} chars, trying DataTransfer paste event...`,
      );
      ...
      const check2 = await cdp.send<{ result: { value: number } }>(
        "Runtime.evaluate",
        {
          expression: `document.querySelector('div[contenteditable="true"]')?.innerText?.length || 0`,
          returnByValue: true,
      ...
      if (check2.result.value > 50) {
        console.log(
          `[weibo-article] Content inserted via DataTransfer (${check2.result.value} chars)`,
        );
      } else {
        console.log(
          `[weibo-article] DataTransfer got ${check2.result.value} chars, trying insertHTML...`,
        );
        ...
        const check3 = await cdp.send<{ result: { value: number } }>(
          "Runtime.evaluate",
          {
            expression: `document.querySelector('div[contenteditable="true"]')?.innerText?.length || 0`,
            returnByValue: true,
        ...
        if (check3.result.value > 50) {
          console.log(
            `[weibo-article] Content inserted via execCommand (${check3.result.value} chars)`,
          );
        } else {
          console.error(
            "[weibo-article] All auto-insert methods failed. HTML is on clipboard - please paste manually (Cmd+V)",
          );
    ...
    if (parsed.contentImages.length > 0) {
      ...
      const editorContent = await cdp.send<{ result: { value: string } }>(
        "Runtime.evaluate",
        {
      ...
      console.log(
        `[weibo-article] ${placeholderCount}/${parsed.contentImages.length} placeholders found in editor`,
      );
      ...
      for (let i = 0; i < sortedImages.length; i++) {
        ...
        console.log(
          `[weibo-article] [${i + 1}/${sortedImages.length}] Inserting image at placeholder: ${img.placeholder}`,
        );
        ...
        const selectPlaceholder = async (maxRetries = 3): Promise<boolean> => {
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            const selectionCheck = await cdp!.send<{ result: { value: string } }>(
              "Runtime.evaluate",
              {
            ...
            if (attempt < maxRetries) {
              console.log(
                `[weibo-article] Selection attempt ${attempt} got "${selectedText}", retrying...`,
              );
              await sleep(500);
            } else {
              console.warn(
                `[weibo-article] Selection failed after ${maxRetries} attempts, got: "${selectedText}"`,
              );
            }
          }
          return false;
        ...
        if (!selected) {
          console.warn(
            `[weibo-article] Skipping image - could not select placeholder: ${img.placeholder}`,
          );
          continue;
        }
        ...
        const stillExists = await cdp!.send<{ result: { value: boolean } }>(
          "Runtime.evaluate",
          {
            expression: `(() => {
            const editor = document.querySelector('div[contenteditable="true"]');
        ...
        if (stillExists.result.value) {
          console.log(
            "[weibo-article] Placeholder survived deleteContents, trying Input.insertText replacement...",
          );
        ...
        const afterDelete = await cdp.send<{ result: { value: boolean } }>(
          "Runtime.evaluate",
          {
            expression: `(() => {
            const editor = document.querySelector('div[contenteditable="true"]');
        ...
        const imgCountBefore = await cdp.send<{ result: { value: number } }>(
          "Runtime.evaluate",
          {
        ...
        if (imgInserted) {
          console.log(
            `[weibo-article] Image insertion verified (${expectedImgCount} image(s) in editor)`,
          );
          ...
          if (altText) {
            ...
            const captionResult = await cdp!.send<{ result: { value: string } }>(
              "Runtime.evaluate",
              {
                expression: `(() => {
                const editor = document.querySelector('div[contenteditable="true"]');
          ...
          if (i === 0) {
            console.error(
              "[weibo-article] First image paste failed. Check Accessibility permissions for your terminal app.",
            );
          }
        }
      ...
      const cleanupResult = await cdp.send<{ result: { value: number } }>(
        "Runtime.evaluate",
        {
          expression: `(() => {
          const editor = document.querySelector('div[contenteditable="true"]');
      ...
      if (cleanupResult.result.value > 0) {
        console.log(
          `[weibo-article] Removed ${cleanupResult.result.value} extra line break(s) before images.`,
        );
      }
      await sleep(500);
      ...
      const finalEditorContent = await cdp.send<{ result: { value: string } }>(
        "Runtime.evaluate",
        {
          expression: `document.querySelector('div[contenteditable="true"]')?.innerText || ''`,
          returnByValue: true,
      ...
      const finalImgCount = await cdp.send<{ result: { value: number } }>(
        "Runtime.evaluate",
        {
          expression: `document.querySelectorAll('div[contenteditable="true"] img').length`,
          returnByValue: true,
      ...
      if (remainingPlaceholders.length > 0 || actualCount < expectedCount) {
        ...
        if (remainingPlaceholders.length > 0) {
          console.warn(
            `[weibo-article]   Remaining placeholders: ${remainingPlaceholders.join(", ")}`,
          );
        }
        if (actualCount < expectedCount) {
          console.warn(
            `[weibo-article]   Image count: expected ${expectedCount}, found ${actualCount}`,
          );
        }
        console.warn("[weibo-article]   Please check the article before publishing.");
      } else {
        console.log(
          `[weibo-article] Verification passed: ${actualCount} image(s), no remaining placeholders.`,
        );
      }
    }
    ...
    if (coverImagePath && fs.existsSync(coverImagePath)) {
      ...
      const coverBtnPos = await cdp.send<{ result: { value: { x: number; y: number } | null } }>(
        "Runtime.evaluate",
        {
          expression: `(() => {
          const el = document.querySelector('.cover-empty') || document.querySelector('.cover-preview');
      ...
      // 2. Click "图片库" tab
      const tabClicked = await cdp.send<{ result: { value: boolean } }>(
        "Runtime.evaluate",
        {
          expression: `(() => {
      ...
      const itemCountBefore = await cdp.send<{ result: { value: number } }>(
        "Runtime.evaluate",
        {
      ...
      const docRoot = await cdp.send<{ root: { nodeId: number } }>(
        "DOM.getDocument",
        { depth: -1 },
        { sessionId },
      );
      const fileInputNodes = await cdp.send<{ nodeIds: number[] }>(
      ...
      if (!fileInputNodeId) {
        ...
        while (Date.now() - uploadStart < 30_000) {
          const state = await cdp.send<{ result: { value: { count: number; firstSrc: string } } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
        ...
        if (!uploadSuccess) {
          ...
          const fallback = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
        ...
        if (uploadSuccess) {
          ...
          const clickResult = await cdp.send<{ result: { value: boolean } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
          ...
          const nextResult = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
          ...
          const confirmInfo = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
              const dialog = document.querySelector('.n-dialog');
          ...
          const confirmClickResult = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
              const dialog = document.querySelector('.n-dialog');
          ...
          // Check dialog state
          const afterConfirm = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
              const dialog = document.querySelector('.n-dialog');
          ...
          if (afterConfirm.result.value !== "closed") {
            ...
            const afterEnter = await cdp!.send<{ result: { value: string } }>(
              "Runtime.evaluate",
              {
          ...
          const coverSet = await cdp.send<{ result: { value: string } }>(
            "Runtime.evaluate",
            {
              expression: `(() => {
