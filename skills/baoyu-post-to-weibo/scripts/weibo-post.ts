...
export async function postToWeibo(options: WeiboPostOptions): Promise<void> {
  ...
  try {
    ...
    if (
      !currentUrl.result.value.includes("weibo.com/") ||
      currentUrl.result.value.includes("card.weibo.com")
    ) {
    ...
    if (text) {
      ...
      const textCheck = await cdp.send<{ result: { value: string } }>(
        "Runtime.evaluate",
        {
      ...
      if (textCheck.result.value.length > 0) {
        ...
        const textRecheck = await cdp.send<{ result: { value: string } }>(
          "Runtime.evaluate",
          {
            expression: `document.querySelector('#homeWrap textarea')?.value || ''`,
            returnByValue: true,
    ...
    if (allFiles.length > 0) {
      ...
      if (uploadCheck.result.value > 0) {
        console.log(
          `[weibo-post] Upload verified (${uploadCheck.result.value} media item(s) detected)`,
        );
      } else {
