...
async function resolveArticleQueryInfo(userAgent: string): Promise<ArticleQueryInfo> {
  ...
  const queryIdMatch = chunk.match(
    /queryId:\"([^\"]+)\",operationName:\"ArticleEntityResultByRestId\"/,
  );
...
async function resolveTweetDetailQueryInfo(userAgent: string): Promise<ArticleQueryInfo> {
  ...
  return {
    ...
    featureSwitches:
      featureSwitches.length > 0 ? featureSwitches : FALLBACK_TWEET_DETAIL_FEATURE_SWITCHES,
...
async function fetchArticleEntityById(
  ...
  const url = new URL(
    `https://x.com/i/api/graphql/${queryInfo.queryId}/ArticleEntityResultByRestId`,
  );
