...
export async function tweetToMarkdown(
  ...
  if (articleEntity) {
    const referencedTweets = await resolveReferencedTweetsFromArticle(articleEntity, cookieMap, {
      log,
    });
...
async function main() {
  ...
  if (!url) {
    console.error("Usage:");
    console.error(
      "  npx -y bun skills/baoyu-danger-x-to-markdown/scripts/tweet-to-markdown.ts <tweet url>",
    );
    process.exit(1);
  }
