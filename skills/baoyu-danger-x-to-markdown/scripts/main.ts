...
function resolveSlugAndId(
  normalizedUrl: string,
  kind: "tweet" | "article",
): { slug: string; idPart: string } {
...
async function convertArticleToMarkdown(
  ...
  const title =
    typeof (article as any)?.title === "string" ? String((article as any).title).trim() : "";
...
async function main(): Promise<void> {
  ...
  if (!articleId && !tweetId) {
    throw new Error(
      "Invalid X url. Examples: https://x.com/<user>/status/<id> or https://x.com/i/article/<id>",
    );
  }
  ...
  if (args.downloadMedia) {
    ...
    if (existingMarkdownPath) {
      ...
      log(
        "[x-to-markdown] Existing markdown already localized; rebuilding content to refresh placement.",
      );
    }
  }
  ...
  const { outputDir, markdownPath, slug } = await resolveOutputPath(
    normalizedUrl,
    kind,
    args.output,
    contentSlug,
