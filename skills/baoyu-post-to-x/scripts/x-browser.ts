  ...
  try {
    ...
    for (const imagePath of images) {
      ...
      if (imgUploadOk) {
        ...
        console.warn(
          "[x-browser] Image upload not detected after 15s. Run check-paste-permissions.ts to diagnose.",
        );
      }
    }
    ...
    if (submit) {
      ...
      console.log(
        "[x-browser] Post composed. Please review and click the publish button in the browser.",
      );
    }
  } finally {
    if (cdp) {
