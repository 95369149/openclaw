...
function preprocessCjkMarkdown(markdown: string): string {
  try {
    const processor = unified().use(remarkParse).use(remarkCjkFriendly).use(remarkStringify);

    const result = String(processor.processSync(markdown));
    return result.replace(/&#x([0-9A-Fa-f]+);/g, (_, hex: string) =>
...
export async function parseMarkdown(
  ...
  let title =
    stripWrappingQuotes(options?.title ?? "") || pickFirstString(frontmatter, ["title"]) || "";
  if (!title) {
  ...
  let coverImagePath =
    stripWrappingQuotes(options?.coverImage ?? "") ||
    pickFirstString(frontmatter, [
      "cover_image",
      "coverImage",
      "cover",
      "image",
