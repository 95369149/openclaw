...
async function checkBun(): Promise<void> {
  ...
  if (result.status === 0) {
    ...
    log(
      "Bun runtime",
      false,
      "Cannot run bun. Install: brew install oven-sh/bun/bun (macOS) or npm install -g bun",
    );
  }
}
...
async function checkRunningChromeConflict(): Promise<void> {
  ...
  if (pids.length > 0) {
    warn(
      ...
      `${pids.length} Chrome process(es) detected. The skill uses --user-data-dir for isolation, so this is safe. Paste keystroke targets Chrome by app name (minor risk if multiple Chrome windows visible).`,
    );
  } else {
