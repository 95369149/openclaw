...
export async function quotePost(options: QuoteOptions): Promise<void> {
  ...
  try {
    ...
    if (!retweetFound) {
      console.log(
        "[x-quote] Tweet not found or not logged in. Please log in to X in the browser window.",
      );
      console.log("[x-quote] Waiting for login...");
      const loggedIn = await waitForRetweetButton();
      if (!loggedIn)
        throw new Error("Timed out waiting for tweet. Please log in first or check the tweet URL.");
    }
...
async function main(): Promise<void> {
  ...
  if (!tweetUrl) {
    ...
    console.error(
      'Example: npx -y bun x-quote.ts https://x.com/user/status/123456789 "Your comment"',
    );
    process.exit(1);
  }
