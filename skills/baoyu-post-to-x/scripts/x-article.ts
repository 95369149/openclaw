...
export async function publishArticle(options: ArticleOptions): Promise<void> {
  ...
  try {
    ...
    let pageTarget = targets.targetInfos.find(
      (t) => t.type === "page" && t.url.startsWith(X_ARTICLES_URL),
    );
    ...
    if (!editorFound) {
      console.log(
        "[x-article] Editor not found. Please ensure you have X Premium and are logged in.",
      );
      await sleep(60_000);
        ...
        if (applyFound) {
          ...
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (attempt < maxRetries) {
              ...
              console.log(
                "[x-article] Modal did not close after all attempts, continuing anyway...",
              );
            }
          }
        } else {
    ...
    const pasteResult = await cdp.send<{ result: { value: boolean } }>(
      "Runtime.evaluate",
      {
        expression: `(() => {
        const editor = document.querySelector('.DraftEditor-editorContainer [contenteditable="true"]');
    ...
    if (contentCheck.result.value > 50) {
      ...
      if (check2.result.value > 50) {
        ...
        console.log(
          "[x-article] Auto-insert failed. HTML copied to clipboard - please paste manually (Cmd+V)",
        );
    ...
    if (parsed.contentImages.length > 0) {
      ...
      for (let i = 0; i < sortedImages.length; i++) {
        ...
        console.log(
          `[x-article] [${i + 1}/${sortedImages.length}] Inserting image at placeholder: ${img.placeholder}`,
        );
        ...
        const selectPlaceholder = async (maxRetries = 3): Promise<boolean> => {
          for (let attempt = 1; attempt <= maxRetries; attempt++) {
            ...
            if (attempt < maxRetries) {
              console.log(
                `[x-article] Selection attempt ${attempt} got "${selectedText}", retrying...`,
              );
              await sleep(500);
            } else {
              console.warn(
                `[x-article] Selection failed after ${maxRetries} attempts, got: "${selectedText}"`,
              );
            }
          }
          return false;
        ...
        if (!selected) {
          console.warn(
            `[x-article] Skipping image - could not select placeholder: ${img.placeholder}`,
          );
          continue;
        }
        ...
        const deleteResult = await cdp.send<{ result: { value: boolean } }>(
          "Runtime.evaluate",
          {
            expression: `(() => {
            const sel = window.getSelection();
        ...
        if (!afterDelete.result.value) {
          console.warn(
            `[x-article] Placeholder may not have been deleted, trying dispatchEvent...`,
          );
        ...
        if (imgUploadOk) {
          ...
          if (i === 0) {
            console.error(
              "[x-article] First image paste failed. Run check-paste-permissions.ts to diagnose.",
            );
          }
        }
      }
      ...
      if (remainingPlaceholders.length > 0 || actualCount < expectedCount) {
        ...
        if (actualCount < expectedCount) {
          console.warn(
            `[x-article]   Image count: expected ${expectedCount}, found ${actualCount}`,
          );
        }
        console.warn("[x-article]   Please check the article before publishing.");
      } else {
        console.log(
          `[x-article] ✓ Verification passed: ${actualCount} image(s), no remaining placeholders.`,
        );
      }
    }
    ...
    const previewClicked = await cdp.send<{ result: { value: boolean } }>(
      "Runtime.evaluate",
      {
        expression: `(() => {
