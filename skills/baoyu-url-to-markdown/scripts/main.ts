...
async function main(): Promise<void> {
  ...
  if (!args.url) {
    console.error(
      "Usage: bun main.ts <url> [-o output.md] [--output-dir dir] [--wait] [--timeout ms] [--download-media]",
    );
    process.exit(1);
  }
  ...
  if (args.output) {
    const stat = await import("node:fs").then((fs) =>
      fs.statSync(args.output!, { throwIfNoEntry: false }),
    );
    if (stat?.isDirectory()) {
  ...
  const outputPath =
    args.output || (await generateOutputPath(args.url, result.metadata.title, args.outputDir));
  ...
  if (args.downloadMedia) {
    ...
    if (mediaResult.downloadedImages > 0 || mediaResult.downloadedVideos > 0) {
      console.log(
        `Downloaded: ${mediaResult.downloadedImages} images, ${mediaResult.downloadedVideos} videos`,
      );
    }
  } else {
