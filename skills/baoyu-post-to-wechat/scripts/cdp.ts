...
export async function launchChrome(
  url: string,
  profileDir?: string,
): Promise<{ cdp: CdpConnection; chrome: ReturnType<typeof spawn> }> {
...
export async function waitForNewTab(
  ...
  while (Date.now() - start < timeoutMs) {
    ...
    const newTab = targets.targetInfos.find(
      (t) => t.type === "page" && !initialIds.has(t.targetId) && t.url.includes(urlPattern),
    );
...
export async function evaluate<T = unknown>(
  ...
  const result = await session.cdp.send<{ result: { value: T } }>(
    "Runtime.evaluate",
    {
      expression,
      returnByValue: true,
