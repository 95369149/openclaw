...
async function postToWeChat(options: WeChatOptions): Promise<void> {
  ...
  if (content.length > 1000)
    throw new Error(`Content too long: ${content.length} chars (max 1000)`);
  if (images.length === 0) throw new Error("At least one image is required");

  const absoluteImages = images.map((p) =>
  ...
  if (!tuWenRef) {
    ...
    ab(
      `eval "document.querySelectorAll('.new-creation__menu .new-creation__menu-item')[2].click()"`,
    );
  } else {
  ...
  const editorTabLine = tabLines.find(
    (l) => l.includes("appmsg") || (!l.includes("cgi-bin/home") && l.includes("mp.weixin.qq.com")),
  );
  ...
  if (titleRef) {
    ...
    ab(
      `eval "const t = document.querySelector('#title'); if(t) { t.value = '${title.replace(/'/g, "\\'")}'; t.dispatchEvent(new Event('input', {bubbles: true})); }"`,
    );
  }
  await sleep(500);
  ...
  const editorRef =
    findElementByText(snapshot, "js_pmEditorArea") || findElementByText(snapshot, "textbox");
  ...
  if (submit) {
    ...
    const submitRef =
      findElementByText(snapshot, "js_submit") || findElementByText(snapshot, "保存");
    if (submitRef) {
      ab(`click ${submitRef}`);
    } else {
