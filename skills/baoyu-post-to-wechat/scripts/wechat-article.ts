...
async function copyImageToClipboard(imagePath: string): Promise<void> {
  ...
  const result = spawnSync("npx", ["-y", "bun", copyScript, "image", imagePath], {
    stdio: "inherit",
  });
...
async function sendCopy(cdp?: CdpConnection, sessionId?: string): Promise<void> {
  if (process.platform === "darwin") {
    ...
    await cdp.send(
      "Input.dispatchKeyEvent",
      { type: "keyDown", key: "c", code: "KeyC", modifiers: 2, windowsVirtualKeyCode: 67 },
      { sessionId },
    );
    await sleep(50);
    await cdp.send(
      "Input.dispatchKeyEvent",
      { type: "keyUp", key: "c", code: "KeyC", modifiers: 2, windowsVirtualKeyCode: 67 },
      { sessionId },
    );
  }
}
...
async function sendPaste(cdp?: CdpConnection, sessionId?: string): Promise<void> {
  if (process.platform === "darwin") {
    ...
    await cdp.send(
      "Input.dispatchKeyEvent",
      { type: "keyDown", key: "v", code: "KeyV", modifiers: 2, windowsVirtualKeyCode: 86 },
      { sessionId },
    );
    await sleep(50);
    await cdp.send(
...
export async function postArticle(options: ArticleOptions): Promise<void> {
  ...
  try {
    ...
    if (!chrome) {
      ...
      const wechatTab =
        loggedInTab ||
        allTargets.targetInfos.find((t) => t.type === "page" && t.url.includes("mp.weixin.qq.com"));
      ...
      if (wechatTab) {
        ...
        if (!currentUrl.includes("/cgi-bin/home")) {
          ...
          await evaluate(
            session,
            `window.location.href = '${WECHAT_URL}cgi-bin/home?t=home/index'`,
          );
          await sleep(5000);
        }
      } else {
