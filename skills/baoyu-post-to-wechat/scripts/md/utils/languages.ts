...
function formatHighlightedCode(html: string, preserveNewlines = false): string {
  ...
  formatted = formatted.replace(/(\s+)(<span[^>]*>)/g, (_: string, spaces: string, span: string) =>
  ...
  if (preserveNewlines) {
    ...
    // 只将空格转换为 &nbsp;
    formatted = formatted.replace(/(>[^<]+)|(^[^<]+)/g, (str: string) =>
      str.replace(/\s/g, `&nbsp;`),
    );
  }
 ...
 * @returns 格式化后的 HTML
 */
export function highlightAndFormatCode(
  text: string,
  language: string,
  hljs: any,
...
export function highlightPendingBlocks(hljs: any, container: Document | Element = document): void {
  ...
  pendingBlocks.forEach((codeBlock) => {
    ...
    if (hljs.getLanguage(language)) {
      ...
      // 动态加载语言后重新高亮
      loadAndRegisterLanguage(language, hljs)
        .then(() => {
          highlightCodeBlock(codeBlock, language, hljs);
        })
        .catch(() => {
