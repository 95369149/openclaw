...
function loadConfig(): WechatConfig {
  ...
  const appSecret =
    process.env.WECHAT_APP_SECRET || cwdEnv.WECHAT_APP_SECRET || homeEnv.WECHAT_APP_SECRET;
...
function renderMarkdownToHtml(
  ...
  console.error(
    `[wechat-api] Rendering markdown with theme: ${theme}${color ? `, color: ${color}` : ""}, citeStatus: ${citeStatus}`,
  );
...
async function main(): Promise<void> {
  ...
  if (args.isHtml) {
    ...
    console.error(
      `[wechat-api] Theme: ${args.theme}${args.color ? `, color: ${args.color}` : ""}, citeStatus: ${args.citeStatus}`,
    );
