          ...
          if (hasSeparators) {
            ...
            if (textChars.length >= rubyParts.length) {
              ...
              for (let i = 0; i < rubyParts.length; i++) {
                ...
                // 提取当前部分的文字
                const currentText = textChars
                  .slice(currentIndex, currentIndex + charCount)
                  .join(``);

                result.push(
                  `<ruby data-text="${currentText}" data-ruby="${rubyPart}" data-format="${format}">${currentText}<rp>(</rp><rt>${rubyPart}</rt><rp>)</rp></ruby>`,
                );
              ...
              for (let i = 0; i < textChars.length; i++) {
                ...
                if (rubyPart) {
                  result.push(
                    `<ruby data-text="${char}" data-ruby="${rubyPart}" data-format="${format}">${char}<rp>(</rp><rt>${rubyPart}</rt><rp>)</rp></ruby>`,
                  );
                } else {
                  result.push(char);
                }
